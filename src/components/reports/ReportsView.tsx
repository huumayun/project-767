...
export const ReportsView: React.FC<ReportsViewProps> = ({ currentSession }) => {
  ...
  const handleExportAllCSV = () => {
    let csvParts: string[] = [];
    const q = (v: string) => `"${(v || '').replace(/"/g, '""')}"`;
    const filename = `consolidated_report_${todayStr}.csv`;

    if (exportSelection.sales && salesReport) {
      csvParts.push("--- SALES REPORT ---");
      csvParts.push(['Date', 'Orders Count', 'Total Sales (Taka)'].join(','));
      salesReport.daily_trends.forEach((t) => {
        csvParts.push([t.date, t.orders_count, (t.sales_paisa / 100).toFixed(2)].join(','));
      });
      csvParts.push(""); // blank line
    }

    if (exportSelection.profit && profitReport) {
      csvParts.push("--- PROFIT REPORT ---");
      csvParts.push(['Product Name', 'Barcode', 'Quantity Sold', 'Revenue (Taka)', 'Cost COGS (Taka)', 'Gross Profit (Taka)', 'Margin %'].join(','));
      profitReport.product_profits.forEach((p) => {
        csvParts.push([
          q(p.product_name),
          p.barcode || '',
          p.qty_sold,
    ...
    if (exportSelection.best_selling && bestSelling) {
      csvParts.push("--- BEST SELLING PRODUCTS ---");
      csvParts.push(['Rank', 'Product Name', 'Barcode', 'Category', 'Quantity Sold', 'Revenue (Taka)'].join(','));
    ...
    if (exportSelection.payments && salesReport) {
      csvParts.push("--- PAYMENTS BREAKDOWN ---");
      csvParts.push(['Payment Channel', 'Total Collected (Taka)'].join(','));
      csvParts.push(['Cash In Drawer', (salesReport.payments_breakdown.cash_paisa / 100).toFixed(2)].join(','));
      csvParts.push(['bKash', (salesReport.payments_breakdown.bkash_paisa / 100).toFixed(2)].join(','));
      csvParts.push(['Nagad', (salesReport.payments_breakdown.nagad_paisa / 100).toFixed(2)].join(','));
      csvParts.push(['Card / Bank POS', (salesReport.payments_breakdown.card_paisa / 100).toFixed(2)].join(','));
      csvParts.push(['Other', ((salesReport.payments_breakdown.other_paisa || 0) / 100).toFixed(2)].join(','));
      csvParts.push("");
    }

    if (exportSelection.due && dueReport) {
      csvParts.push("--- DUE / BAKI REPORT ---");
      csvParts.push(['Type', 'Name', 'Phone', 'Billed (Taka)', 'Paid (Taka)', 'Outstanding (Taka)', 'Last Sale'].join(','));
      dueReport.receivables.forEach((c) => {
        csvParts.push([
          'Customer owes shop',
          q(c.name),
          q(c.phone || ''),
      ...
      dueReport.payables.forEach((sup) => {
        csvParts.push([
          'Shop owes supplier',
          q(sup.name),
          q(sup.phone || ''),
    ...
    if (exportSelection.valuation && stockValuation) {
      csvParts.push("--- STOCK VALUATION ---");
      csvParts.push(['Metric', 'Value'].join(','));
      csvParts.push(['Total Unique Items', stockValuation.total_products_count].join(','));
      csvParts.push(['Total Stock Units', stockValuation.total_stock_units].join(','));
      csvParts.push(['Total Cost Valuation (Taka)', (stockValuation.total_cost_valuation_paisa / 100).toFixed(2)].join(','));
      csvParts.push(['Total Retail Valuation (Taka)', (stockValuation.total_retail_valuation_paisa / 100).toFixed(2)].join(','));
      csvParts.push(['Potential Gross Profit (Taka)', (stockValuation.potential_gross_profit_paisa / 100).toFixed(2)].join(','));
      csvParts.push("");
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-3xl border border-jungle-teal-200 shadow-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3.5 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-semibold rounded-xl text-ui-xs flex items-center gap-1.5 transition-colors cursor-pointer"
            title="Export only this tab"
          >
          ...
          <button
            ...
            title="Export multiple reports at once"
          >
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="bg-jungle-teal-50 p-5 border-b border-jungle-teal-100">
              <h3 className="text-lg font-bold text-jungle-teal-900 tracking-tight flex items-center gap-2">
                <Download className="w-5 h-5 text-emerald-600" />
                Custom CSV Export
              </h3>
              <p className="text-xs text-jungle-teal-700 mt-1">Select the reports you want to include in the consolidated CSV file.</p>
            </div>
            <div className="p-5 space-y-3">
              {[
                { key: 'sales', label: 'Sales Report' },
                { key: 'profit', label: 'Profit Report' },
                { key: 'best_selling', label: 'Best Selling Products' },
                { key: 'payments', label: 'Payment Channels' },
                { key: 'due', label: 'Due / Baki Report' },
                { key: 'valuation', label: 'Stock Valuation' },
              ].map((item) => (
                <label key={item.key} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:bg-slate-50 cursor-pointer transition-colors">
                  <input
                    type="checkbox"
                    checked={exportSelection[item.key as keyof typeof exportSelection]}
                    onChange={(e) => setExportSelection({ ...exportSelection, [item.key]: e.target.checked })}
                    className="w-4 h-4 rounded text-jungle-teal-600 focus:ring-jungle-teal-500 border-slate-300"
                  />
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex gap-3 justify-end">
              <button
                ...
                className="px-4 py-2 font-semibold text-slate-700 bg-white border border-slate-300 hover:bg-slate-100 rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 font-semibold text-white bg-jungle-teal-600 hover:bg-jungle-teal-700 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
